---
title: "XYZ to Text Files and Lidar Pre-Processing -- R Version"
author: "Lindi Oyler"
date: "7/16/2021"
output:
  html_document:
...
# 1. Purpose
This code uses ### R SCRIPT ### and combines all CUBE .xyz and CUBE Uncertainty .xyz for each lake survey into one file and removing overlappling/non-matching points. More specifically, the list of files is read in from the directory. Regardless of the number of CUBE files in the directory, it will remove the cross check file and read in the uncertainty file separately while combining all of the xyz files into one data frame.This also binds the xyz and uncertainty columns while removing duplicates and creates two new columns for QA and Map.

Secondly, this code combines LiDAR .las files and the NED DEM GeoTIFF file and converts them to .csv and text files.

Lastly, this code performs the same combining, filtering, and file conversion operations on the cross-check (XCheck) files and outputs .txt and .csv files into the RTextFiles folder.

# 2. Function Definitions and Running the Program
##################################################################################################
```{r}
...
## This program requires 5 unique inputs from the user: the home lake directory, the lake name, the dam/spillway elevation (in meters), the water surface elevation (in meters), and the names of any alternative source types (ADCP, GPS, SB, etc). Note: the alternative source types should be entered as a list. "Alternative source type" refers to anything that is not multibeam data. For example:

*alt_source_types = c("ADCP","SB")* #if there are two or more alternative source types

OR

*alt_source_types = "ADCP"* #if there is one alternative source type

OR

*alt_source_types = c()* #if there are NO alternative source types

### The outputs are text files and csv files in the RTextFiles folder containing the XYZ_uncert_source information, the Lidar text files and csv files that contains its XYZ information, and text files and csv files of the cross-check (XCheck) points xyz locations.Printed on the screen will be QA map point counts and other details necessary for documentation and QA processes. Enter these in the QAMapPointCounts spreadsheet. 

### Do NOT forget to click Knit at the top of the screen after the program runs! This is necessary for documentation. I promise you'll thank me for it when you need this information again in 6 months. The HTML output of the knitting will be stored in the source folder for this code. Change the name of the output to include the lake name and then _XYZtoTextFilesandLidarProcessing.html and move it into the lake's RTextFiles folder. For example:

GardenCityOld_XYZtoTextFilesandLidarProcessing.html
