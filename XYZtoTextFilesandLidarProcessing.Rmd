...
XYZToTextFiles<-function(home_dir, lake_name, WSEL, dam_elev){
  gc()
  ...
  #########
  #Looping through the ADCP files (if they exist) and concatenating them. This loop is skipped if they do not exist
  print("Working on ADCP files...")
  if(dir.exists(adcp_path)){
    ...
    for(i in 1:length(ls)){
      ...
      if(i==1){
        adcp=subset(adcp,select=c("UTM_X","UTM_Y","Z"))
  ...
  #########
  #Looping through the GPS files (if they exist) and concatenating them. This loop is skipped if they do not exist
  print("Working on GPS files...")
  if(dir.exists(gps_path)){
    setwd(gps_path)
    i=1
    ls=list.files(pattern="GPS.txt$")
    for(i in 1:length(ls)){
      ...
      if(i==1){
        ...
        gps=subset(gps,select=c("UTM_X","UTM_Y","Z","Bed_elev"))
        colnames(gps)=c("X","Y","Z")
        temp=gps
        gps_all=plyr::rbind.fill(all,temp)
      }
  ...
  } ##Adding ADCP points into the overall dataframe if they exist
  
  if(dir.exists(gps_path)){
    df=plyr::rbind.fill(df, gps_all)
  } ##Adding GPS points into the overall dataframe if they exist
...
LidarToTextFiles <- function(home_dir, lake_name, WSEL, dam_elev){
  ...
  setwd(lidar_path)
  ls=list.files(lidar_path,pattern=".laz$")
  ...
  if(length(ls)!=0){
    ...
    for(i in 1:length(ls)){
      ...
      las=lasfilterduplicates(las) #Removing duplicate points
      ...
      utm15crs <- crs("+proj=utm +zone=15 +ellps=GRS80 +units=m +no_defs")
      temp_las <- st_as_sf(las_all,coords=c("X","Y"),crs=utm15crs)
      las_bbox <- st_bbox(temp_las,xmin=min(las_all$X), xmax=max(las_all$X),
                          ymin=min(las_all$Y), ymax=max(las_all$Y),crs=utm15crs)
  ...
  elev=las_all[,3]
  clip_las=subset(las_all,elev<=(dam_elev + 3)) #Selects Lidar points that are less than 3 meters above the dam/spillway
...
## This program requires 4 inputs from the user: the home lake directory, the lake name, the dam/spillway elevation (in meters), and the water surface elevation (in meters). The output is the text files containing the XYZ_uncert_source information and the Lidar text file that contains its XYZ information.Printed on the screen will be QA map point counts and other details necessary for documentation and QA processes. Enter these in the QAMapPointCounts spreadsheet. Do NOT forget to click Knit at the top of the screen after the program runs! This is necessary for documentation. I promise you'll thank me for it when you need this information again in 6 months.

#Sit back, relax, and watch the magic happen...

```{r}

XYZToTextFiles(home_dir = "E:/HYPACK_Projects/2020_DNR_Lakes/2020-07_Harrisonville",
                                 lake_name = "Harrisonville",
                                 WSEL =272.998,
                                 dam_elev =273.576)


LidarToTextFiles(home_dir = "E:/HYPACK_Projects/2020_DNR_Lakes/2020-07_Harrisonville",
                                 lake_name = "Harrisonville",
                                 WSEL =272.998,
                                 dam_elev =273.576)
